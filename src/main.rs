...
fn focus_nth_selectable(nth: usize) {
    ...
    first_element
        .set_attribute("nav-index", &nth.to_string())
        .unwrap();
    first_element.focus();
}
...
fn select_move(direction: DIRECTION) {
    ...
    let new_idx = ((current_idx as i32 + dir + all_selectable_cnt as i32)
        % all_selectable_cnt as i32) as usize;
    unselect_element();
...
fn softkey_set_label(left: &str, center: &str, right: &str) {
    document()
        ...
        .unwrap()
        .unwrap()
        .set_text_content(left);
    document()
        .query_selector("#center")
        .unwrap()
        .unwrap()
        .set_text_content(center);
    document()
        .query_selector("#right")
        .unwrap()
        .unwrap()
        .set_text_content(right);
}
...
fn main() -> Fallible<()> {
    ...
    document().add_event_listener(move |event: KeyPressEvent| match event.key().as_str() {
        "Enter" => event_enter(),
